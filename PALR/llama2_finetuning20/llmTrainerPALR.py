...
# 최대 토큰 길이 설정 
max_input_length = 3000 # 최대 토큰 길이 확인 후 설정
...
class FinetuningDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        full_text = (
            ...
            f"User has purchased the following products {', '.join(entry['input']['interaction'])}\n"
            f"The candidates are {', '.join(entry['input']['candidates'])} [/INST]\n"
            f"{', '.join(entry['output']['recommended'])}"
        )
...
code_dataset = "namejun12000/PALR_finetuning20"
