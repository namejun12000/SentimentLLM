...
# 50 % (test)
dataset_50 = load_dataset("namejun12000/PALR_inference1", split="train_50_second")

# 50 % (valid)
datset_50_valid = load_dataset("namejun12000/PALR_inference2", split="train_50_second")

# # 전체 데이터 중에서 0.05%만 샘플링 (test)
# sampled_dataset = dataset_50.shuffle(seed=52).select(range(25))
...
def evaluate_loo(model, dataset, tokenizer, top_k=10):
    ...
    for example in tqdm(dataset, desc="Evaluating LOO", unit="user"):
        ...
        preference = example["input"]["preference"]
        ground_truth = example["output"]["recommended"]
        if len(interaction_history) < 5:
            continue

        # LOO 분할
        test_item = interaction_history[-1]
        valid_item = interaction_history[-2]
        train_items = interaction_history[:-1]

        # 입력 준비
        input_text = (
            f"[INST] Recommend 10 other items based on user's history from the candidate list.\n"
            f"{preference}\n"
            f"The user's history interactions are:\n {', '.join(map(str, train_items))}"
            f"\nThe candidates are: {', '.join(map(str, candidates))} [/INST]\nOutput: "
        )
        inputs = tokenizer(input_text, return_tensors="pt").to(model.device)

        # 모델 예측
        with torch.no_grad():
            outputs = model.generate(**inputs, max_new_tokens=400, temperature=0.8, top_k=10, top_p=0.85)
            ...
            recommended_text = re.search(r"Output:\s*(.*)", generated_text)
            if recommended_text:
                # Match alphanumeric product IDs (e.g., B004TSFE6Y)
                recommended_items = re.findall(r"\bB[A-Z0-9]+\b\s*\(.*?\)", recommended_text.group(1))
                # Filter only items in the candidate list
                recommended_items = [item for item in recommended_items if item in candidates]
            else:
                recommended_items = []
